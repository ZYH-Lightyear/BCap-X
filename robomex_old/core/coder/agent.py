...
class CodingAgent:
            ...
            while True:
                ...
                with event_scope(turn=turn_idx):
                    ...
                    if tool_call.name == "use_skill":
                        ...
                        loaded_before = set(loaded)
                        message = self._load_skill_message(name, loaded)
                        engine.append_feedback(prompt, message)
                        loaded_now = set(loaded) != loaded_before
                        emit_event(
                            "skill_loaded" if loaded_now else "skill_load_rejected",
                            (
                                f"Loaded skill {name}"
                                if loaded_now
                                else f"Did not load skill {name}"
                            ),
                            skill=name,
                        ...
                        feedback = self._feedback_message(execution)
                        if isinstance(feedback, list):
                            self._drop_prior_inline_images(prompt)
                        engine.append_feedback(prompt, feedback)
    ...
    def _load_skill_message(self, name: str, loaded: list[str]) -> str:
        available = {entry.name for entry in self._skill_entries()}
        if name not in available:
            avail = ", ".join(sorted(available)) or "(none)"
            return (
                f"No available skill named '{name}'. Available skills: {avail}. "
                "Choose only from <available_skills>."
            )
        if name in loaded:
            return (
                f"Skill '{name}' is already loaded in this agent session. "
                "Use its guidance and tools instead of loading it again."
            )
        try:
    ...
    def _ensure_skill_contract_bindings(
        ...
        if cached is not None:
            return cached
        from robomex.skills.contract import load_contract_for_skill, resolve_function_signature
    ...
    def _feedback_message(execution: BlockExecutionResult) -> str | list:
        ...
        info = execution.info if isinstance(execution.info, dict) else {}
        ...
        if tool_results:
            ...
            tool_feedback = (
                "tool_results (exact return values; use these field names and paths, "
                "do not invent artifact names):\n"
                f"{_compact_stream_for_prompt('tool_results', rendered, limit=6000)}\n\n"
            )
        text = (
            f"stdout:\n{stdout}\n\nstderr:\n{stderr}\n\n{terminal_state}"
            f"{tool_feedback}"
        ...
        for raw in image_paths:
            ...
            if key in seen or not resolved.is_file():
                continue
            mime = mimetypes.guess_type(resolved.name)[0] or ""
            if not mime.startswith("image/"):
                continue
            # 视觉反馈保持轻量；工具仍会在文本结果中返回所有 artifact 路径。
            if resolved.stat().st_size > 8 * 1024 * 1024:
                continue
            from robomex.perception.render import image_content_part

            content.append(image_content_part(resolved, max_edge=1280))
            seen.add(key)
            if len(seen) >= 4:
                break
    ...
    @staticmethod
    def _drop_prior_inline_images(prompt: list[dict]) -> None:
        """Keep only the newest visual tool result in the growing conversation."""
        ...
        for message in prompt:
            ...
            if len(compacted) != len(content):
                message["content"] = compacted or [
                    {
                        "type": "text",
                        "text": "[Earlier tool images omitted; artifact paths remain in text.]",
                    }
                ]
