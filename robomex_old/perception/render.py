...
def image_content_part(path: str | Path, *, max_edge: int | None = None) -> dict:
    """Build an OpenAI-compatible image part, optionally using a light VLM preview.

    The artifact on disk remains untouched.  When ``max_edge`` is set, large or
    unusually heavy images are converted to a bounded RGB JPEG in memory so one
    tool turn cannot flood every later model request with multi-megabyte PNG data.
    """
    ...
    if max_edge is not None:
        if isinstance(max_edge, bool) or max_edge < 64:
            raise ValueError("max_edge must be at least 64")
        with Image.open(source) as opened:
            image = opened.convert("RGB")
            if max(image.size) > max_edge:
                image.thumbnail((max_edge, max_edge), Image.Resampling.LANCZOS)
            if len(raw) > 512 * 1024 or image.size != opened.size:
                buffer = io.BytesIO()
