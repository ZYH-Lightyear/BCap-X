...
def run_codex_exec_streaming(
    *,
    ...
    """Run Codex CLI and stream raw JSONL events to stdout while capturing them."""
    command = [
        codex_bin,
        "exec",
        "--model",
    ...
    while True:
        ...
        if deadline is not None and now >= deadline:
            process.kill()
            timed_out = True
            stderr += f"\nCodex killed after timeout_s={timeout_s}.\n"
            break
        if not line:
            time.sleep(0.1)
...
def run_claude_code(
    *,
    ...
    """Run a single autonomous Claude Code print-mode session."""
    command = [
        claude_bin,
        "-p",
        "--model",
    ...
    try:
        ...
        if isinstance(stderr, bytes):
            stderr = stderr.decode(errors="replace")
        stderr += f"\nClaude Code killed after timeout_s={timeout_s}.\n"
...
def run_claude_code_streaming(
    *,
    ...
    """Run Claude Code print mode and stream JSONL events while capturing them."""
    command = [
        claude_bin,
        "-p",
        "--model",
...
def run_coding_agent(
    *,
    ...
    if agent == "codex":
        if stream:
            return run_codex_exec_streaming(
                workspace=workspace,
                model=model,
                prompt=prompt,
                codex_bin=codex_bin,
    ...
    if agent == "claude":
        if stream:
            return run_claude_code_streaming(
                workspace=workspace,
                model=model,
                prompt=prompt,
